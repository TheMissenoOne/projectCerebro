---
type: personagem
nome: "Johnny Dee"
aliases: ["Johnny Dee"]
nome_civil: "desconhecido"
mutante: true
origem: canonico
status_2010: ativo
status_poder: ativo
idade_2010: ~35
idade_estimada: true
primeira_aparicao_campanha: "Pré-2004"
geracao: "Sobrevivente próximo ao Instituto"
equipe_2010: []
afiliacoes: ["Instituto Xavier (próximo)"]
local_2010: "Refugiado"
mentor: ""
alunos: []
familia: []
poderes: ["Duplicação de aparência", "Disfarce"]
nivel_ameaca: baixo
segredo: false
fonte: estabelecido
tags: ["sobreviventes","198","manteve-poderes-dia-m"]
---

# Johnny Dee

## Resumo

Mutante com capacidade de duplicar aparência (disfarce), listado entre os sobreviventes próximos ao Instituto.

## Poderes

- Duplicação de aparência.
- Disfarce.

> [!QUESTION]
> Detalhes do poder — pendente.

## Ver também

- [[Contagem dos 198]]